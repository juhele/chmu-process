...
## max air temperature
files<-list.files(pattern = "*_TMA_N.csv$") # load files based on pattern
...
airTmax<-data.frame(date=cas)
airTmax$date<-as.character(airTmax$date)
...
for (i in 1:length(files)) {
  ...
  airTmax<-merge(airTmax,tms,by="date",all.x = T) # final merge
}

save(airTmax,file="airTmax_merged.RData")
